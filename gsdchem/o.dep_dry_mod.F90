...
!  use chem_types_mod, only : kind_phys
...
contains
    ...
    subroutine dry_dep_driver(ktau,dtstep,current_month,t_phy,p_phy,      &
               moist,p8w,rmol,alt,t8w,raincv,                             &
               chem,rho_phy,dz8w,exch_h,hfx,                              &
               ivgtyp,tsk,gsw,vegfra,pbl,ust,znt,z,z_at_w,                &
               xland,xlat,xlong,ddep,g,numgas,                            &
               num_chem,num_moist,                                        &
...
!
!c  REAL(kind_phys), DIMENSION( its:ite, jts:jte ) ::   aer_res
!c  REAL(kind_phys), DIMENSION( its:ite, jts:jte ) ::   aer_res_def
!c  REAL(kind_phys), DIMENSION( its:ite, jts:jte ) ::   aer_res_zcen
...
!c      IF( chem_opt /= GOCART_SIMPLE ) THEN
!c         call wesely_driver(ktau,dtstep,                                 &
!c              current_month,                                              &
!c              gmt,julday,t_phy,moist,p8w,t8w,raincv,                      &
!c              p_phy,chem,rho_phy,dz8w,ddvel,aer_res_def,aer_res_zcen,    &
!c              ivgtyp,tsk,gsw,vegfra,pbl,rmol,ust,znt,xlat,xlong,z,z_at_w,&
!c              snowh,numgas,                                                    &
!c              ids,ide, jds,jde, kds,kde,                                 &
!c              ims,ime, jms,jme, kms,kme,                                 &
!c              its,ite, jts,jte, kts,kte                                  )
!c      ENDIF
       print*,'hli in dry_dep_driver'
