...
!!
   ...
   subroutine skinsst_run(				&
    ...
    dt_cool,		& ! sub-layer cooling amount			out
...
! --- output:
   real (kind=kind_phys), dimension(:), intent(out) :: evap, hflx,	&
      ep, qsat, gflux, cmm, chh, dt_warm, dt_cool, z_c
      character(len=*), intent(out) :: errmsg
      integer,          intent(out) :: errflg

! --- locals:
   integer :: i, n, loop
   real :: alon, alat, virt, rho_air, rho_wat, pvap, tsq,		&
     nonsol,			& ! sum of nonsolar air-sea fluxes
     logdplt,			& ! log(1./dplete)
...
!     'sfcemis',sfcemis(i),		& ! sfc emissivity
      ...
      'plyr1',plyr1(i),			& ! layer 1 presure
      'tlyr1',tlyr1(i)-frz,		& ! layer 1 air temp
      'qlyr1',qlyr1(i),			& ! layer 1 humidity
...
! --- and the reflected part is (1-sfcemis)*dlwflx/sfcemis
     ...
     if (doprint(alon,alat))						&
      ...
      'ulwflx',ulwflx(i),		& ! upwelling LW flux
      'dt_cool',dt_cool(i),		& ! skin temperature increment
